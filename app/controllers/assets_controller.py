...
class AssetsController():
    ...
    def get_type_by_asset_id(self, asset_id:int, db:Session):
    ...
    def get_asset_by_id(self, asset_id:int, db:Session):
        query = select(self.model).where(self.model.id == asset_id)
        asset = db.exec(query).first()
        if not asset:
            return None
    ...
    def get_asset_price_by_id(self, asset_id:int, db:Session):
        ...
        if asset_type == 'stock':
            ...
            current_price = CoingeckoController().get_current_coin_price_by_name(name=coin.name)
